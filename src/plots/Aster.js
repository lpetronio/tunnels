...
export default class Aster {
  ...
  //      this.sanityCheck(data); // check data for all necessary attributes 
    ...
    updateDimension() {
        ...
        this.dimension.height = d3.max(this.data.map(d=>d.y)) * ((outerRadius*2) + asterPadding) + this.padding.top + this.padding.bottom;

        this.dimension.innerWidth = this.dimension.width - this.padding.left - this.padding.right;
        this.dimension.innerHeight = d3.max(this.data.map(d=>d.y)) * ((outerRadius*2) + asterPadding);
    ...
    createScales(){
        console.log(this.data[0].values.map(d=> d.ancestry))
        ...
        this.scale = {
            ...
            node: {
                color: d3.scaleSequential().domain([0,1]).interpolator(YLOrRd)
            },
            series: {
            ...
            .each(function(d){
                ...
                const renderNode = ()=>{
                    d3.select(this)
                        ...
                        .attr("cy", 0)
                        .attr("r", innerRadius)
                        .attr("fill", scale.node.color(d.node.color))
                };
                ...
                const renderSeries = ()=>{
                    ...
                    var arc = d3.arc()
                        ...
                        .outerRadius( (e)=> { 
                            return ((outerRadius - innerRadius) * (e.data.outerRadius) + innerRadius) - 5.5;
                        });
                    ...
                    pathG
                        ...
                        .each(function(d){
                            ...
                            let renderLabel = ()=>{
                                ...
                                .attr("fill", d=> scale.series.customColor(d.data.color))
